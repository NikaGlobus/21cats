...
# Цикл ввода группы: пусто = все группы, иначе проверяем формат и наличие файла группы
while true; do
    read -p "Введите номер группы (например A-06-04, Enter — для всех групп): " GROUP
    
    # Пустой ввод = анализ по всем группам
    if [ -z "$GROUP" ]; then
        GROUP=""
        break
    fi
    ...
    if ! [[ "$GROUP" =~ ^(A|Ae)-[0-9]{2}-[0-9]{2}$ ]]; then
        echo "Ошибка: номер группы введён неверно! Примеры: A-09-22 или Ae-21-21."
        continue
    fi

    # ИСПРАВЛЕНИЕ: правильный путь к файлам групп - students/groups/
    if [ ! -f "$ROOT_DIR/students/groups/$GROUP" ]; then
        echo "Ошибка: группы '$GROUP' нет в базе (файл $ROOT_DIR/students/groups/$GROUP не существует)."
        continue
    fi

    # Если дошли сюда — группа корректна
    break
done
...
# ---------- Выбор вывода ----------
read -p "Выводить результаты в файл? (y - в файл, n/ENTR - в консоль): " save_to_file
...
change_settings() {
	...
	while true; do
   		 ...
   		 # ИСПРАВЛЕНИЕ: правильный путь к файлам групп - students/groups/
    		if [ ! -f "$ROOT_DIR/students/groups/$GROUP" ]; then
        		echo "Ошибка: группы '$GROUP' нет в базе."
        		continue
    		fi
